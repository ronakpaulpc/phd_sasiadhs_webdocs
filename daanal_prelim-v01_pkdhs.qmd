---
title: "PKDHS preliminary data analysis"
---
...
In this document we do preliminary analysis of examining infant mortality clustering under mothers in the pooled Pakistan Demographic and Health Survey (PKDHS) datasets. This document has selected code for analysis and the full spectrum of preliminary analysis are available in the "pkdhs_datanal1.R" script file.

## Data source

Here we use the pooled dataset of 4 existing rounds of Pakistan Demographic and Health Survey (PKDHS) conducted during 1990, 2006, 2012 and 2017. The PKDHS implemented a multi-stage cluster sampling design. Details regarding the sample design, survey instruments, informed consent, training of interviewers and other fieldwork officers, data collection and processing, and response rates are available in the final report of the respective surveys.

We pooled the raw birth histories of ever-married women in the reproductive age group (15-49 years) across all the pre-specified rounds. We manually harmonized all required datasets so that our study variables have consistent categorisation and label codes across the surveys. The pooled dataset harmonization and pooling codes are in "pkdhsv2_daprep.R". Also, we have a variable harmonization documentation called "pkdhs_datapl1_pre.html" which shows how the variable labels and label codes vary across the raw PKDHS datasets.

Now, please note that the following Pakistan DHS surveys were pooled:
...
pkbr1_tmp_intro |> select(c(ctr_name, svy_year, n_births)) |> 
    # Join vars from pkhr_tmp_intro
    left_join(
        pkhr1_tmp_intro |> select(svy_year, n_households),
        by = join_by(svy_year)
    ) |> 
    # Join vars from pkpr_tmp_intro
    left_join(
        pkpr1_tmp_intro |> select(svy_year, n_persons),
        by = join_by(svy_year)
    ) |> 
...
# required further
rm(list = c("pkbr1_tmp_intro", "pkhr1_tmp_intro", "pkpr1_tmp_intro"))
...
# Importing the pooled pkbr tibble
pkbr1_pl1 <- read_rds(
...
We will check the distribution of births by background characteristics in Pakistan 1970-2022.
...
t1_ut <- pkbr1_pl1 |> 
    ...
    # Table title
    # modify_caption("**Table S1: Sample distribution of births by background characteristics in Pakistan 1970-2022**") |> 
    bold_labels()
t1_ut
...
#| echo: true
#| output: true
#| tbl-cap: "Distribution of births by background characteristics in Pakistan 1970-2022"
...
@tbl-univariate-dist-utwt presents the sample characteristics of all births from the complete birth histories of Pakistan from 1970 to 2022. The distribution of our two main explanatory variables, shows that there were 11% children whose previous sibling had died before their birth. Most children in Pakistan had a preceding birth interval of 24 months or more. Among the children considered in the study, the majority were from the 1990-99 birth cohort (35.5%), followed by the 2000-09 cohort (23.1%) and the 1980-89 cohort (22.7%). More than half of the children were male (51.2%) and belonged to the first or second birth order (56.4%).

Almost 38% of children were born when their mothers were aged 20-24 years, followed by 24% whose mothers were less than 20 years old. Nearly 71% of mothers and 36% of fathers had no formal schooling. Most households practised Hinduism (84.8%), and Pakistani was the primary language spoken in almost 46% of households. The majority of children lived in rural areas (75.7%) and in the Terai region (52.7%), followed by the Hill region (40%) and the Mountain region (7.3%).
...
#| echo: true
#| output: true
#| fig-cap: "Lollipop chart showing the weighted percentage of infant deaths by birth cohort in Pakistan DHS"
...
#| echo: true
#| output: true
#| fig-cap: "Lollipop chart showing the weighted percentage of infant deaths by survey period in Pakistan DHS"
...
Here we calculate mortality rates across the Pakistan DHS survey rounds using the DHS.Rates R package. First we prepared the dataset of childhood mortality rates and then we drew the graph.
...
pkbrpl1_svyround_nest <- pkbr1_pl1 |> 
    nest(.by = year) 

# Second, we calculate the child mortality rates for each tibble
pkbrpl1_svyround_nest <- pkbrpl1_svyround_nest |> 
...
pkbrpl1_mortrates <- pkbrpl1_svyround_nest |> 
    select(-data) |> 
...
#| fig-width: 8
#| fig-height: 8
#| fig-cap: "Connected dot plot showing the childhood mortality rates by Pakistan DHS survey rounds" 

# Now we prepare the connected scatter plots of nnmr, imr and u5mr
fig2_svyround <- pkbrpl1_mortrates |> 
    ...
    # add labels
    labs(
        title = "Trends in Childhood mortality rates in Pakistan DHS survey rounds",
        x = "Survey round",
...
#| output: true
#| warning: false
#| tbl-cap: "Distribution of infant deaths by background characteristics among Pakistani children from 1970-2022"
...
#| echo: true
#| output: true
#| tbl-cap: "Multivariable association of infant mortality and the independent variables among children in Pakistan 1970-2022"
